function cNames = Rgb2ElevenColourNames(cfext,im, do3D)
    ...
    if isempty(colourNameMatrix)  % Check if it is empty (i.e. not initialized)
        load(fullfile(vsemRoot,'data/colornames/elevenColourNames.mat'));  %# Initialize data with the .MAT file contents
