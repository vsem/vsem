function m = DiagMatrixLinear(a,b)
% m = DiagMatrix(a, b)
%
...
%     assignment = ([1:nP nP:-1:1] - 0.5 )/ nP;
    assignment = NormalizeRows([1:nP nP:-1:1] - 0.5 );
    ...
    m(1:1.5*nP,1) = NormalizeRows(firstAssignment);
    lastAssignment = ones(1, a - iB + 1);
    lastAssignment(1,1:nP) = ((1:nP) - 0.5) / nP;
    m(iB:end,b) = NormalizeRows(lastAssignment);    
else % uneven
%     assignment = [1:nP nP-1:-1:1] / nP;
    assignment = NormalizeRows([1:nP nP-1:-1:1]);
    ...
    m(1:(nP + (nP-1)/2)) = NormalizeRows(firstAssignment);
    lastAssignment = ones(1, a - iB + 1);
